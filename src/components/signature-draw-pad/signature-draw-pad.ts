...
@Component({
  ...
  templateUrl: 'signature-draw-pad.html'
})
export class SignatureDrawPadComponent {
	  ...
	  constructor() {
		console.log('Hello SignatureDrawPadComponent Component');
	...
	public initSignaturePad(options: IOptions) {
		...
		if (this.throttle) {
		  this._strokeMoveUpdate = throttle(
			SignatureDrawPadComponent.prototype._strokeUpdate,
			this.throttle,
		  );
		} else {
		  this._strokeMoveUpdate = SignatureDrawPadComponent.prototype._strokeUpdate;
		}

		this.dotSize =
		  options.dotSize ||
		  function dotSize(this: SignatureDrawPadComponent) {
	  ...
	  private _fromData(
		...
		drawCurve: SignatureDrawPadComponent['_drawCurve'],
		drawDot: SignatureDrawPadComponent['_drawDot'],
	  ): void {
